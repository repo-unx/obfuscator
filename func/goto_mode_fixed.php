...
function convertToGoto($code) {
    ...
    $labelPrefix = generateRandomString(3, 5);
...
function createAdvancedGotoObfuscation($code) {
    ...
    $varCounter = generateRandomString(4, 8);
    $varLimit = generateRandomString(3, 7);
    
    // Start building results
    $result = '';
    
    // Generate function names
    $cleanerFunctionName = generateRandomString(5, 10);
    ...
    for ($i = 0; $i < $labelCount; $i++) {
        $realLabels[] = $labelPrefix . '_' . generateRandomString(3, 5);
    }
    
    // Create fake labels (for junk code)
    for ($i = 0; $i < $labelCount * 2; $i++) {
        $fakeLabels[] = $labelPrefix . '_' . generateRandomString(3, 5);
    }
